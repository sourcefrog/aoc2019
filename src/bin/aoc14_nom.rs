...
type IngredientMap = BTreeMap<Chemical, u64>;
...
fn parse(s: &str) -> RecipeMap {
    let (rest, r) = all_consuming(map(
        many1(map(parse_recipe, |r| (r.product.chemical.clone(), r))),
        |rs| rs.into_iter().collect::<RecipeMap>(),
    ))(s)
    .expect("parse error");
...
fn parse_quantity(s: &str) -> IResult<&str, Quantity> {
    map(
        separated_pair(
            map_res(digit1, |d: &str| d.parse::<u64>()),
            tag(" "),
            map(alpha1, str::to_owned),
        ),
        |(n, chemical)| Quantity { n, chemical },
    )(s)
}

fn parse_ingredient_list(s: &str) -> IResult<&str, IngredientMap> {
    map(separated_list1(tag(", "), parse_quantity), |v| {
        v.into_iter().map(|r| (r.chemical, r.n)).collect()
    })(s)
}
...
fn parse_recipe(s: &str) -> IResult<&str, Recipe> {
    map(
        preceded(
            multispace0,
            terminated(
                separated_pair(parse_ingredient_list, tag(" => "), parse_quantity),
                multispace0,
            ),
        ),
        |v| Recipe {
